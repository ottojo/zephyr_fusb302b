...
static const uint8_t BIT_CONTROL1_RX_FLUSH = 2;
...
static int i2c_set_bit(const struct fusb302b_cfg *cfg, uint8_t reg, uint8_t bit_index)
{
	uint8_t prev_value = 0;
	int res = i2c_reg_read_byte_dt(&cfg->i2c, reg, &prev_value);
	if (res != 0) {
		return res;
	}

	res = i2c_reg_write_byte_dt(&cfg->i2c, reg, prev_value | (1 << bit_index));
	return res;
}
 ...
 */
static int fusb302b_get_rx_pending_msg(const struct device *dev, struct pd_msg *buf)
{
	// TODO: Return codes!
...
static int flush_rx_buffer(const struct fusb302b_cfg *cfg)
{
	return i2c_set_bit(cfg, REG_CONTROL1, BIT_CONTROL1_RX_FLUSH);
}
...
int transmit_sop(const struct fusb302b_cfg *cfg, struct pd_msg *msg)
{
	...
	// TODO: do we need to do something else to enable tx driver again? does this disable it?
	// apparently no goodcrc after tx anymore!
...
int fusb302b_enable_rx(const struct device *dev, bool enable)
{

	// return 0;
	LOG_WRN("fusb302b_enable_rx %d", enable);

	const struct fusb302b_cfg *cfg = dev->config;

	int res = flush_rx_buffer(cfg);
	if (res != 0) {
		return -EIO;
	}
	...
	// Enable auto-crc(?)
	uint8_t switches1 = 0;
	res = i2c_reg_read_byte_dt(&cfg->i2c, REG_SWITCHES1, &switches1);
	if (res != 0) {
		return -EIO;
	}
	...
	res = i2c_reg_write_byte_dt(&cfg->i2c, REG_SWITCHES1, switches1);
	if (res != 0) {
		return -EIO;
	}
...
static const struct tcpc_driver_api fusb302b_tcpc_driver_api = {
	...
	.set_rx_enable = fusb302b_enable_rx,
